import '../add_session_page/add_session_page_widget.dart';
...
import '../session_detail_page/session_detail_page_widget.dart';
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => AddSessionPageWidget(),
            ),
          );
        },
        backgroundColor: FlutterFlowTheme.of(context).primaryColor,
                        ...
                        children: [
                          StreamBuilder<FormationsRecord>(
                            stream: FormationsRecord.getDocument(
                                listViewSessionsRecord.formation!),
                            builder: (context, snapshot) {
                              // Customize what your widget looks like when it's loading.
                              if (!snapshot.hasData) {
                                return Center(
                          ...
                          Text(
                            dateTimeFormat(
                                'MMMMEEEEd', listViewSessionsRecord.dateD!),
                            style: FlutterFlowTheme.of(context).subtitle1,
                          ),
                          StreamBuilder<List<SessionSRecord>>(
                            stream: querySessionSRecord(
                            ...
                            builder: (context, snapshot) {
                              ...
                              List<SessionSRecord>
                                  iconButtonSessionSRecordList = snapshot.data!;
                              // Return an empty Container when the document does not exist.
                              if (snapshot.data!.isEmpty) {
                                return Container();
                              }
                              final iconButtonSessionSRecord =
                                  iconButtonSessionSRecordList.isNotEmpty
                                      ? iconButtonSessionSRecordList.first
                                      : null;
